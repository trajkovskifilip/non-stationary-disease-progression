...
data = pd.read_csv('controlled_scenario.tsv', sep='\t')
...
# First split: 60% trainig, 40% remaining
X_train, X_temp, y_train, y_temp, sample_weights_train, sample_weights_temp, time_since_onset_train, time_since_onset_temp = train_test_split(
    X, y, sample_weights, time_since_onset,
    train_size=0.6,
    random_state=42)

# Split remaining data equally into validation and test set (20% / 20%)
X_val, X_test, y_val, y_test, sample_weights_val, sample_weights_test, time_since_onset_val, time_since_onset_test = train_test_split(
    X_temp, y_temp, sample_weights_temp, time_since_onset_temp,
    test_size=0.5,
    random_state=42)
...
def train_and_eval(model_builder, model_name):
    ...
    # Evaluate
    test_metrics, subscores_metrics, phase_metrics = evaluate_model_equal_visits(
        best_model, X_test, y_test, sample_weights_test
    )
